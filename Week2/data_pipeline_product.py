...
def load_data_to_db(ti):
    ...
    target_metadata.create_all(engine, tables=[products_table], checkfirst=True)

    with engine.begin() as conn:
        conn.execute(products_table.insert(), data_to_load)

    engine.dispose()
    print(f"Loaded {len(data_to_load)} records into '{products_table.name}'.")

# --- DAG ---
with DAG(
    dag_id='api_to_db_pipeline_products',
