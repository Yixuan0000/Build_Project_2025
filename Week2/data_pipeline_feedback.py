...
def fetch_data_from_api_callable():
    endpoint = f"{API_BASE_URL}/feedback/"  # Replace "some_endpoint" appropriately
...
def load_data_to_db(ti):
    ...
    for row in data_to_load:
        row["feedback_form_sent_date"] = format_datetime(row.get("feedback_form_sent_date"))
        row["feedback_answer_date"] = format_datetime(row.get("feedback_answer_date"))


    db_url = f"mysql+pymysql://{MYSQL_USERNAME}:{MYSQL_PASSWORD}@{MYSQL_HOST}:{MYSQL_PORT}/{MYSQL_DB_NAME}"
    engine = create_engine(db_url)

    target_metadata.create_all(engine, tables=[feedback_table], checkfirst=True)

    with engine.begin() as conn:
       conn.execute(feedback_table.insert(), data_to_load)

    engine.dispose()
    print(f"Loaded {len(data_to_load)} records into '{feedback_table.name}'.")

# --- Define the DAG ---
with DAG(
    dag_id='api_to_db_pipeline_feedback',
